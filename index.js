...
var ASM_CONSTS = {
  71209: ($0) => { if (Module.printNoNewline) { Module.printNoNewline(UTF8ToString($0)); } else { console.log(UTF8ToString($0)); } }
};
function __asyncjs__js_init_fs() { return Asyncify.handleAsync(async () => { if (typeof FS !== 'undefined') { if (!FS.analyzePath('/data').exists) { FS.mkdir('/data'); } FS.mount(IDBFS, {}, '/data'); console.log("Loading from IndexedDB..."); await new Promise((resolve, reject) => { FS.syncfs(true, (err) => { if (err) { console.error("IDBFS Load Error:", err); resolve(); } else { console.log("Load complete."); resolve(); } }); }); } }); }
function __asyncjs__js_sync_to_db() { return Asyncify.handleAsync(async () => { if (typeof FS !== 'undefined') { console.log("Saving to IndexedDB..."); await new Promise((resolve, reject) => { FS.syncfs(false, (err) => { if (err) { console.error("IDBFS Sync Error:", err); reject(err); } else { console.log("Save complete."); resolve(); } }); }); } }); }
function __asyncjs__js_get_input() { return Asyncify.handleAsync(async () => { const result = await Module.waitForBrowserInput(); var lengthBytes = lengthBytesUTF8(result) + 1; var stringOnWasmHeap = _malloc(lengthBytes); stringToUTF8(result, stringOnWasmHeap, lengthBytes); return stringOnWasmHeap; }); }
...
var wasmImports = {
  /** @export */
  __asyncjs__js_get_input,
  /** @export */
  __asyncjs__js_init_fs,
  /** @export */
  __asyncjs__js_sync_to_db,
  /** @export */
